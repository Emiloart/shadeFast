...
class _GlobalFeedScreenState extends ConsumerState<GlobalFeedScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Global Hot'),
        actions: <Widget>[
          IconButton(
            onPressed: () => context.push('/polls'),
            tooltip: 'Trending Polls',
      ...
      body: feed.when(
        ...
        error: (Object error, StackTrace _) => _FeedErrorState(
          ...
          onRetry: () => ref.invalidate(globalFeedControllerProvider),
        ),
        data: (FeedPageState state) {
          ...
          return RefreshIndicator(
            onRefresh: () => ref
                .read(globalFeedControllerProvider.notifier)
                .refreshFromTop(),
            child: NotificationListener<ScrollNotification>(
              onNotification: (ScrollNotification notification) {
                if (notification.metrics.pixels >=
                    notification.metrics.maxScrollExtent - 300) {
                  ref.read(globalFeedControllerProvider.notifier).loadMore();
                }
                return false;
              },
              child: ListView.separated(
                ...
                itemBuilder: (_, int index) {
                  ...
                  return _FeedPostCard(
                    ...
                    onToggleLike: () => _toggleLike(ref, post),
...
Future<void> _toggleLike(WidgetRef ref, ShadePost post) async {
  ...
  try {
    ...
    ref.read(globalFeedControllerProvider.notifier).refreshFromTop();
  } catch (_) {
